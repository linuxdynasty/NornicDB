...
**Status:** Proposed
**Date:** April 15, 2026
**Scope:** Replace hardcoded memory-tier decay behavior with a generic, policy-driven decay and scoring system that can support existing, proposed, or future decay models.
...
The system should instead treat decay behavior as configurable retention policy.
...
- resolve effective policy from configuration and policies
...
- schema-backed policies as the main control surface
- diagnostics that explain why a given entity resolved to a given policy

### 4.2 Policy Subsystem Layer

The policy subsystem is the authoring surface for retention behavior.
...
- support property-targeted rules in addition to node and relationship targets

Suggested fit in NornicDB:

- introduce a dedicated decay-policy subsystem with its own catalog and DDL
- borrow authoring, validation, and introspection patterns from the constraint subsystem without making decay rules first-class constraints
- express property-level retention as inline policy entries inside a policy body
- add retention-specific resolution rules alongside existing schema rules

### 4.3 Runtime Resolution Layer

The runtime resolution layer converts configuration and policies into an effective policy for an entity or property.
...
- resolve inline property entries from the active policy before falling back to entity defaults
...
Logical rule compiled from policy definitions and used by the resolver.
...
2. property-level inline rule inside the applicable policy
3. entity-level rule inside the applicable policy
...
For Cypher scoring access, a missing policy should not be treated as an error. If no explicit policy or configured default applies, the target should resolve as non-decaying with a neutral full score of `1.0`.
...
- which policy and inline rule selected it
...
Decay behavior should be authored through a dedicated policy subsystem, not through first-class constraints.
...
- inline property paths on relationships within a policy body

### 7.3 Policy Semantics

If decay is globally disabled, the policies still exist in schema but are operationally inactive until the subsystem is enabled.
...
Property-level retention rules should be authored inline within the same policy body that declares the entity-level defaults for that label or relationship type.

Nested `FOR ... APPLY` entries should remain invalid inside a policy body. If operators need retention rules for a different label or relationship type, they should create a separate targeted policy.
...
In this model, property-level rules are just targeted entries in the same policy body. They should not require a separate `CREATE POLICY` statement unless the target label or relationship type itself changes.

### 7.5 Policy DDL

Policies should be first-class database objects in a dedicated subsystem, not ordinary graph nodes and not first-class constraints.

Operators should be able to define policies independently and bind them via target selectors in the policy statement itself.
...
- implement policy compilation for decay-aware entries
- support node-, relationship-, and property-targeted policies
...
4. Define and implement schema-backed policy entries for nodes and relationships in the dedicated policy subsystem.
5. Extend policy parsing and compiled policy metadata to support property-targeted retention entries.
...
- explain output identifies the exact policy entry and effective policy
...
- `decayScore(...)` returns `1.0` rather than an error when no policy or configured default applies
...
- operators can define decay semantics through config and policies
- operators can define property-level retention inline in policy bodies
...
- new decay models can be expressed as policies without new engine categories
...
- dedicated policy-subsystem support for inline property-level retention rules
...
Named presets may remain in documentation for bootstrapping a memory decay model for operator convenience, but the engine should ultimately care only about effective decay policy.
